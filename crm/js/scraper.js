                ...
                try {
                    ...
                    if (resp.ok) {
                        ...
                        for (const f of data.features || []) {
                            ...
                            // 1. Street / Road / Place / Infrastructure / Transit / Residential / Retail exclusion
                            if (p.type === 'street' || p.type === 'district' || p.type === 'city' || p.type === 'locality' || p.osm_key === 'highway' || p.osm_key === 'place' || p.osm_key === 'natural' || p.osm_key === 'railway' || p.osm_key === 'aeroway' || p.osm_key === 'historic' || p.osm_key === 'shop' || p.osm_key === 'healthcare') continue;
                            if (p.shop || p.healthcare || p.tourism || p.leisure) continue;
                            if (/^(شارع|طريق|كوبرى|محور|ميدان|تقاطع|نزلة|طلعة|موقف|نفق|محطة|جبل|خط |عين |حدائق)/.test(rawName)) continue;

                            // 2. Non-commercial exclusions (OSM tags)
                            const nonCommercialKeys = ['cemetery', 'police', 'university', 'school', 'kindergarten', 'hospital', 'clinic', 'pharmacy', 'place_of_worship', 'mosque', 'church', 'residential', 'motorway', 'trunk', 'drinking_water', 'events_venue', 'resort', 'hotel', 'chalet', 'courthouse', 'townhall', 'government', 'public_building', 'social_facility', 'community_centre', 'library', 'post_office', 'bank', 'atm', 'fast_food', 'restaurant', 'cafe', 'bar', 'ice_cream', 'nightclub', 'cinema', 'theatre', 'swimming_pool', 'sports_centre', 'fitness_centre', 'stadium', 'park', 'playground', 'garden', 'fire_station', 'prison', 'recycling', 'mall', 'supermarket', 'bakery', 'beauty', 'clothes', 'pastry', 'confectionery', 'car_wash', 'fuel', 'guest_house', 'hostel', 'motel'];
                            if (nonCommercialKeys.includes(p.osm_value) || nonCommercialKeys.includes(p.amenity) || nonCommercialKeys.includes(p.tourism) || nonCommercialKeys.includes(p.leisure) || nonCommercialKeys.includes(p.shop)) continue;

                            // 3. Comprehensive Arabic name-based non-B2B filter (all spelling variants)
                            if (/(مجمع محاكم|محكمة|محكمه|محاكم|النيابة العامة|نيابة|نيابه|مجلس الدولة|الشهر العقاري|مصلحة الضرائب|مصلحه الضرائب|مأمورية ضرائب|مامورية ضرائب|مصلحة الجمارك|مصلحه الجمارك|مبنى ادار|مبني ادار|مبنى إدار|مبني إدار|العهد الجديد|ديوان عام|ديوان المحافظ|الوحدة المحلية|الوحده المحليه|مجلس مدينة|مجلس مدينه|مجلس قروي|مكتب تموين|سجل مدني|سجل مدنى|مكتب بريد|سنترال|هيئة الأبنية|هيئه الابنيه|الشئون الاجتماعية|التضامن الاجتماعي|مكتب صحة|مكتب صحه|وحدة صحية|وحده صحيه|مدرسة|مدرسه|مدارس|حضانة|حضانه|روضة|روضه|جامعة|جامعه|كلية|كليه|معهد أزهري|معهد ازهري|معهد موسيقي|معهد موسيقى|سنتر تعليمي|أكاديمية تعليمية|اكاديمية تعليمية|هندسة حلوان|هندسة عين شمس|هندسة القاهرة|طب القاهرة|طب عين شمس|كلية الهندسة|كلية الطب|كلية التجارة|مستشفى|مستشفي|مستشفا|عيادة|عياده|عيادات|مركز طبي|مركز طبى|مركز عيون|مركز أشعة|مركز اشعة|مركز علاج|مركز أسنان|مركز اسنان|للعيون|للأسنان|للاسنان|للأشعة|للاشعة|صيدلية|صيدليه|صيدليات|مستوصف|مختبر تحاليل|معمل تحاليل|مسجد|جامع الن|جامع ال|مسجد ال|كنيسة|كنيسه|كاتدرائية|كاتدرائيه|دير الأنبا|دير الشهيد|دير القديس|دير السريان|دير المحرق|دير مار|دير وادي|مطرانية|مطرانيه|خلوة|خلوه|بيت الخلوة|بيت الخلوه|بيت المؤتمرات|بيت الضيافة|بيت الضيافه|بيت الشباب|بيت القيثارة|بيت القيثاره|بيت ثقافة|بيت ثقافيه|جمعية خيرية|جمعيه خيريه|مؤسسة خيرية|مؤسسه خيريه|دار أيتام|دار ايتام|دار مسنين|دار رعاية|قسم شرطة|قسم شرطه|نقطة شرطة|نقطه شرطه|مركز شرطة|مركز شرطه|إدارة مرور|ادارة مرور|أمن مركزي|امن مركزي|معسكر|سجن|قاعدة جوية|قاعده جويه|مركز شباب|نادي رياضي|نادى رياضى|نادي اجتماعي|نادى اجتماعى|حديقة عامة|حديقه عامه|حدائق|ملعب|استاد|مقابر|مقبرة|مقبره|جبانة|جبانه|مدافن|مغسلة اموات|مغسله اموات|فرن بلدي|فرن بلدى|مخبز بلدي|مخبز بلدى|مخبز|حلواني|حلوانى|باتيسري|مطعم|كافيه|كافيتريا|كوفي شوب|بيتزا|كشري|فول وطعمية|مشويات|شاورما|كبابجي|اسماك|كبدة|سوبر ماركت|ميني ماركت|هايبر ماركت|محل بقالة|محل بقاله|محل خضار|محل فاكه|محل جزارة|محل دواجن|عطارة|عطاره|مقلة|مقله|محمصة|مول |shopping mall|كوافير|صالون حلاقة|صالون حلاقه|صالون رجالي|صالون حريمي|بيوتي سنتر|beauty salon|دراي كلين|dry clean|مغسلة ملابس|مغسله ملابس|خياط|ترزي|اتيليه|ميك اب|مكتبة عامة|مكتبه عامه|قرطاسية|ادوات مكتبية|ادوات كتابية|بلايستيشن|جيم |فتنس|سفارة|سفاره|قنصلية|قنصليه|ماكينة صراف|ماكينه صراف|ATM|صراف آلي|فرع بنك|خدمات فوري|أمان للمدفوعات|دار مناسبات|قاعة افراح|قاعة افراح|فوتوسيشن|ستوديو تصوير|استوديو تصوير|عربية كبدة|عربيه كبده|عربية فول|عربيه فول|كشك |بائع |محل موبايل|صيانة موبايل|صيانة شاشات|موقف توشكى|موقف ميكروباص|محطة مترو|محطة قطار|محطة اتوبيس)/.test(rawName)) continue;
