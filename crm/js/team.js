...
const Team = {
    ...
    renderEmployeesPage() {
        try {
            ...
            const users = window.AppStorage.getUsers() || [];
            const pendingUsers = window.AppStorage.getPendingUsers();
            ...
            <table class="data-table" style="width:100%; border-collapse:collapse;">
                ...
                <tbody>
                    ${assignedCompanies.map(c => {
                        let statusBadge = '';
                        if (c.lastCallResult) {
                            statusBadge = `<span class="result-badge result-${c.lastCallResult}">${window.AppStorage.getCallResultLabel(c.lastCallResult)}</span>`;
                            ...
                            <tr style="border-bottom:1px solid var(--border-light);">
                                ...
                                <td style="padding:10px; font-size:12px;">
                                    <span class="badge" style="background:var(--bg-surface); border:1px solid var(--border-color); padding:2px 8px;">
                                        ${window.AppStorage.getSectorLabel(c.sector)} | ${window.AppStorage.getCityLabel(c.city)}
                                    </span>
                                </td>
    ...
    openUserModal(userId = null) {
        let user = null;
        if (userId) {
            user = window.AppStorage.getUser(userId);
        }
    ...
    async saveUser() {
        ...
        if (id) {
            ...
            const res = await window.AppStorage.updateUser(id, updatePayload);
            if (!res.success) {
            ...
            const res = await window.AppStorage.addUser({ firstName, lastName, email, password, erpCode, region, role });
            if (!res.success) {
                App.showToast(`❌ ${res.message}`, 'error');
                return;
            }
    ...
    openResetPasswordModal(userId) {
        ...
        document.getElementById('form-reset-password-submit').onsubmit = async (e) => {
            ...
            const res = await window.AppStorage.resetUserPassword(userId, newPassword);
            if (!res.success) {
                App.showToast(`❌ ${res.message}`, 'error');
                return;
            }
    ...
    deleteUser(id) {
        ...
        document.getElementById('btn-confirm-action').onclick = () => {
            const res = window.AppStorage.deleteUser(id);
    ...
    renderAssignList(targetUserId) {
        ...
        const totalAssignedToTarget = window.AppStorage.getCompanies().filter(c => c.assignedTo === targetUserId).length;
        if (countEl) {
    ...
    approveUser(userId, role = 'agent') {
        const user = window.AppStorage.approveUser(userId, role);
        if (user) {
    ...
    rejectUser(userId) {
        ...
        document.getElementById('btn-confirm-action').onclick = () => {
            window.AppStorage.rejectUser(userId);
            App.closeModal('modal-confirm');
    ...
    toggleFreeze(userId) {
        const user = window.AppStorage.toggleUserFreeze(userId);
        if (user) {
