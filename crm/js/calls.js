...
const Calls = {
    ...
    renderStats() {
        ...
        const calls = CRM.getScopedCalls();
    ...
    getCallAgentName(call) {
        ...
        if (call.userId) {
            const u = CRM.getUser(call.userId);
            if (u) return u.name;
        }
        if (call.companyId) {
            const c = CRM.getCompany(call.companyId);
            if (c && c.assignedTo) {
                const u = CRM.getUser(c.assignedTo);
                if (u) return u.name;
            }
        }
        return 'غير محدد';
    ...
    renderTable() {
        ...
        const calls = CRM.getScopedCalls().sort((a, b) => {
    ...
    renderGroupedByCompany(calls, currentUser, container) {
        ...
        groupsMap.forEach((companyCalls, compId) => {
            const company = CRM.getCompany(compId);
            const companyName = company ? esc(company.nameAr || company.nameEn) : 'شركة غير معروفة';
            const sectorLabel = company ? CRM.getSectorLabel(company.sector) : '—';
            const cityLabel = company ? CRM.getCityLabel(company.city) : '—';
                            ...
                            ${companyCalls.map(call => {
                                ...
                                <div style="padding: 14px 20px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px; border-bottom: 1px solid var(--border-light); transition: background 0.15s ease;" onmouseover="this.style.background='rgba(99,102,241,0.04)'" onmouseout="this.style.background='transparent'">
                                    <div style="display: flex; align-items: center; gap: 14px; min-width: 260px;">
                                        <span class="result-badge result-${call.result}" style="font-size: 12px; padding: 4px 10px;">${CRM.getCallResultLabel(call.result)}</span>
    ...
    renderFlatTable(calls, currentUser) {
        ...
        tbody.innerHTML = pageCalls.map(call => {
            ...
            const resultLabel = CRM.getCallResultLabel(call.result);
            const agentName = this.getCallAgentName(call);
    ...
    populateCompanyDropdown(selectId, selectedCompanyId = '') {
        ...
        companies.forEach(c => {
            ...
            html += `<option value="${c.id}" ${isSelected}>${name} (${CRM.getSectorLabel(c.sector)})</option>`;
        });
