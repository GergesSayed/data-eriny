/**
 * scripts/verify_v261.js
 * Comprehensive integrity verification for v261.0 release:
 * 400 Titans + 24,928 Pool = 25,328 Total Companies.
 */
...
console.log('=== RUNNING COMPREHENSIVE V261.0 INTEGRITY VERIFICATION ===\n');
...
check('Titans count is exactly 400', titans.length === 400, `Found: ${titans.length}`);
...
check('All 400 Titans have strictly sequential IDs eg_titan_001 to eg_titan_400', idsMatchSequence);
check('All 400 Titan IDs are unique', titanIds.size === 400);
...
check('All 400 Titans have complete mandatory attributes (isTitan, priority A+, fleet specs)', schemaValid);
check('Total combined fleet of 400 Titans is verified', totalTitanFleet > 80000, `Total Fleet: ${totalTitanFleet}`);
...
check('egypt_verified_titans.js contains eg_titan_400', titansJsContent.includes('eg_titan_400'));
...
check('Total companies.json count is exactly 25,328 (400 + 24,928)', companies.length === 25328, `Found: ${companies.length}`);

const allCompanyIds = new Set();
for (const c of companies) {
  allCompanyIds.add(c.id);
}
check('All 25,328 company IDs in companies.json are unique', allCompanyIds.size === 25328);

// First 400 must be Titans
let first400AreTitans = true;
for (let i = 0; i < 400; i++) {
  if (companies[i].id !== `eg_titan_${String(i + 1).padStart(3, '0')}`) {
    first400AreTitans = false;
    break;
  }
}
check('First 400 companies in companies.json are the 400 Titans', first400AreTitans);

// 5. Version checks in index.html & storage.js
const indexHtml = fs.readFileSync(indexHtmlPath, 'utf8');
check('index.html specifies CURRENT_VER 261.0', indexHtml.includes("var CURRENT_VER = '261.0';"));
check('index.html specifies count 25328 in storage check', indexHtml.includes("localStorage.setItem('fleetcrm_company_count', '25328');"));
check('index.html specifies count 25,328 in fallback UI', indexHtml.includes("'25,328'"));
check('index.html references script bundles with ?v=261.0', indexHtml.includes('js/egypt_verified_titans.js?v=261.0'));

const storageJs = fs.readFileSync(storageJsPath, 'utf8');
check('storage.js uses worker cache buster ?v=261.0', storageJs.includes("js/companies-worker.js?v=261.0"));

console.log('\n--- Sector Breakdown of 400 Titans ---');
