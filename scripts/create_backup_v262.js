/**
 * scripts/create_backup_v262.js
 * Comprehensive Backup for v262.0 (500 Titans + 24,928 Pool = 25,428 Companies).
...
const backupDir = path.join(__dirname, `../backups/companies_backup_v262_${date}`);
...
console.log(`Starting v262.0 backup: ${companies.length} companies (${titans.length} Titans)...`);
...
fs.writeFileSync(path.join(backupDir, 'companies_v262.json'), JSON.stringify(companies, null, 2), 'utf8');
const fullCsv = generateCsv(companies);
fs.writeFileSync(path.join(backupDir, 'companies_v262.csv'), fullCsv, 'utf8');
const fullXlsx = generateXlsxXml(companies, 'All Companies v262');
fs.writeFileSync(path.join(backupDir, 'companies_v262.xlsx'), fullXlsx, 'utf8');

fs.writeFileSync(path.join(backupDir, 'titans_500_v262.json'), JSON.stringify(titans, null, 2), 'utf8');
const titansCsv = generateCsv(titans);
fs.writeFileSync(path.join(backupDir, 'titans_500_v262.csv'), titansCsv, 'utf8');
const titansXlsx = generateXlsxXml(titans, '500 Titans');
fs.writeFileSync(path.join(backupDir, 'titans_500_v262.xlsx'), titansXlsx, 'utf8');
...
fs.writeFileSync(path.join(latestDir, 'companies_master_25428.json'), JSON.stringify(companies, null, 2), 'utf8');
fs.writeFileSync(path.join(latestDir, 'companies_master_25428.csv'), fullCsv, 'utf8');
fs.writeFileSync(path.join(latestDir, 'companies_master_25428.xlsx'), fullXlsx, 'utf8');

fs.writeFileSync(path.join(latestDir, 'egypt_verified_titans_500.json'), JSON.stringify(titans, null, 2), 'utf8');
fs.writeFileSync(path.join(latestDir, 'egypt_verified_titans_500.csv'), titansCsv, 'utf8');
fs.writeFileSync(path.join(latestDir, 'egypt_verified_titans_500.xlsx'), titansXlsx, 'utf8');
...
const metadataMd = `# 🛡️ Fleet CRM Master Database - Backup Metadata (v262.0)

**Backup Creation Date:** 2026-09-21  
**System Version:** \`v262.0\`  
**Total Companies:** **25,428**  
**Total Industrial Titans (Tier 1 & Tier 2 Heavyweight Enterprises):** **500**  
**Total Verified Enterprise Pool:** **24,928**  
**Titans Combined Fleet:** **${totalFleet.toLocaleString()} heavy commercial vehicles**

---

## 🏢 Sector Breakdown of the 500 Titans:
${Object.entries(sectorMap).sort((a, b) => b[1] - a[1]).map(([sec, cnt]) => `- **${sec}:** ${cnt} Titans`).join('\n')}

---

## 📁 Available Backup Formats:
1. **JSON:** \`companies_master_latest.json\` / \`companies_master_25428.json\` / \`egypt_verified_titans_500.json\`
2. **CSV (UTF-8 BOM for Microsoft Excel):** \`companies_master_latest.csv\` / \`companies_master_25428.csv\` / \`egypt_verified_titans_500.csv\`
3. **XLSX (Native Spreadsheet):** \`companies_master_latest.xlsx\` / \`companies_master_25428.xlsx\` / \`egypt_verified_titans_500.xlsx\`

---

## 🔒 Integrity & Quality Assurance:
- Sequential IDs \`eg_titan_001\` through \`eg_titan_500\`.
