/**
 * Create v256 backup - JSON, CSV (UTF-8 BOM), XLSX
 */
const fs = require('fs');
const path = require('path');

const date = new Date().toISOString().slice(0, 10).replace(/-/g, '');
const backupDir = path.join(__dirname, `../backups/companies_backup_v256_${date}`);
if (!fs.existsSync(backupDir)) fs.mkdirSync(backupDir, { recursive: true });

const companies = JSON.parse(fs.readFileSync('crm/data/companies.json', 'utf8'));

// 1. JSON backup
fs.writeFileSync(path.join(backupDir, 'companies_v256.json'), JSON.stringify(companies, null, 2), 'utf8');
...
fs.writeFileSync(path.join(backupDir, 'companies_v256.csv'), BOM + csvRows.join('\n'), 'utf8');
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="Companies v256">
  <Table>`;
...
fs.writeFileSync(path.join(backupDir, 'companies_v256.xlsx'), xmlHeader + '\n' + xmlRows + xmlFooter, 'utf8');
console.log(`✅ XLSX backup: ${companies.length} rows`);

console.log(`\n📁 All backups saved to: ${backupDir}`);
console.log(`   - companies_v256.json`);
console.log(`   - companies_v256.csv`);
console.log(`   - companies_v256.xlsx`);
