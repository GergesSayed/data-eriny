/**
 * Verify v259.0 dataset integrity
 */
const fs = require('fs');
...
check('Total companies', companies.length === 25143, `${companies.length} (expected 25143)`);
check('Titans count in companies.json', titansInCompanies.length === 215, `${titansInCompanies.length} (expected 215)`);
check('Titans count in egypt_verified_titans.json', titansJson.length === 215, `${titansJson.length} (expected 215)`);
...
check('index.html version', /CURRENT_VER\s*=\s*'259\.0'/.test(html), 'CURRENT_VER = 259.0');
check('index.html query strings', /\?v=259\.0/.test(html), 'Has ?v=259.0');

// Version in storage.js
check('storage.js version', /\?v=259\.0/.test(storage), 'Has ?v=259.0');
...
console.log(`\n${'='.repeat(40)}`);
if (pass) {
  console.log('  🎉 ALL CHECKS PASSED - v259.0 VERIFIED');
} else {
