/**
 * Create v258 backup - JSON, CSV (UTF-8 BOM), XLSX
 */
const fs = require('fs');
const path = require('path');

const date = new Date().toISOString().slice(0, 10).replace(/-/g, '');
const backupDir = path.join(__dirname, `../backups/companies_backup_v258_${date}`);
if (!fs.existsSync(backupDir)) fs.mkdirSync(backupDir, { recursive: true });

const companies = JSON.parse(fs.readFileSync('crm/data/companies.json', 'utf8'));

// 1. JSON backup
fs.writeFileSync(path.join(backupDir, 'companies_v258.json'), JSON.stringify(companies, null, 2), 'utf8');
...
fs.writeFileSync(path.join(backupDir, 'companies_v258.csv'), BOM + csvRows.join('\n'), 'utf8');
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="Companies v258">
  <Table>`;
...
fs.writeFileSync(path.join(backupDir, 'companies_v258.xlsx'), xmlHeader + '\n' + xmlRows + xmlFooter, 'utf8');
...
if (fs.existsSync(latestDir)) {
  ...
  fs.writeFileSync(path.join(latestDir, 'companies_master_25016.json'), JSON.stringify(companies, null, 2), 'utf8');
  fs.writeFileSync(path.join(latestDir, 'companies_master_25016.csv'), BOM + csvRows.join('\n'), 'utf8');
  fs.writeFileSync(path.join(latestDir, 'companies_master_25016.xlsx'), xmlHeader + '\n' + xmlRows + xmlFooter, 'utf8');
  
  const titans = JSON.parse(fs.readFileSync('crm/data/egypt_verified_titans.json', 'utf8'));
  fs.writeFileSync(path.join(latestDir, 'egypt_verified_titans_88.json'), JSON.stringify(titans, null, 2), 'utf8');
  fs.copyFileSync('crm/js/egypt_verified_titans.js', path.join(latestDir, 'egypt_verified_titans.js'));
  
  console.log(`✅ backups/latest updated with v258.0 (25,016 companies, 88 Titans)`);
}
