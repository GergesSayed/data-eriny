/**
 * scripts/create_backup_v263.js
 * Comprehensive Backup for v263.0 (700 Titans + 24,928 Pool = 25,628 Companies).
...
const backupDir = path.join(__dirname, `../backups/companies_backup_v263_${date}`);
...
console.log(`Starting v263.0 backup: ${companies.length} companies (${titans.length} Titans)...`);
...
fs.writeFileSync(path.join(backupDir, 'companies_v263.json'), JSON.stringify(companies, null, 2), 'utf8');
const fullCsv = generateCsv(companies);
fs.writeFileSync(path.join(backupDir, 'companies_v263.csv'), fullCsv, 'utf8');
const fullXlsx = generateXlsxXml(companies, 'All Companies v263');
fs.writeFileSync(path.join(backupDir, 'companies_v263.xlsx'), fullXlsx, 'utf8');

fs.writeFileSync(path.join(backupDir, 'titans_700_v263.json'), JSON.stringify(titans, null, 2), 'utf8');
const titansCsv = generateCsv(titans);
fs.writeFileSync(path.join(backupDir, 'titans_700_v263.csv'), titansCsv, 'utf8');
const titansXlsx = generateXlsxXml(titans, '700 Titans');
fs.writeFileSync(path.join(backupDir, 'titans_700_v263.xlsx'), titansXlsx, 'utf8');
...
fs.writeFileSync(path.join(latestDir, 'companies_master_25628.json'), JSON.stringify(companies, null, 2), 'utf8');
fs.writeFileSync(path.join(latestDir, 'companies_master_25628.csv'), fullCsv, 'utf8');
fs.writeFileSync(path.join(latestDir, 'companies_master_25628.xlsx'), fullXlsx, 'utf8');

fs.writeFileSync(path.join(latestDir, 'egypt_verified_titans_700.json'), JSON.stringify(titans, null, 2), 'utf8');
fs.writeFileSync(path.join(latestDir, 'egypt_verified_titans_700.csv'), titansCsv, 'utf8');
fs.writeFileSync(path.join(latestDir, 'egypt_verified_titans_700.xlsx'), titansXlsx, 'utf8');
...
const metadataMd = `# 🛡️ Fleet CRM Master Database - Backup Metadata (v263.0)

**Backup Creation Date:** 2026-09-21  
**System Version:** \`v263.0\`  
**Total Companies:** **25,628**  
**Total Industrial Titans (100% Complete Directory of Egyptian Titans):** **700**  
**Total Verified Enterprise Pool:** **24,928**  
**Titans Combined Fleet:** **${totalFleet.toLocaleString()} heavy commercial vehicles**

---

## 🏢 Sector Breakdown of the 700 Titans:
${Object.entries(sectorMap).sort((a, b) => b[1] - a[1]).map(([sec, cnt]) => `- **${sec}:** ${cnt} Titans`).join('\n')}

---

## 📁 Available Backup Formats:
1. **JSON:** \`companies_master_latest.json\` / \`companies_master_25628.json\` / \`egypt_verified_titans_700.json\`
2. **CSV (UTF-8 BOM for Microsoft Excel):** \`companies_master_latest.csv\` / \`companies_master_25628.csv\` / \`egypt_verified_titans_700.csv\`
3. **XLSX (Native Spreadsheet):** \`companies_master_latest.xlsx\` / \`companies_master_25628.xlsx\` / \`egypt_verified_titans_700.xlsx\`

---

## 🔒 Integrity & Quality Assurance:
- Sequential IDs \`eg_titan_001\` through \`eg_titan_700\`.
